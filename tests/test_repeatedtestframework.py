...
class ParameterisedTestResult(unittest.TestResult):
    def __init__(self):
        self._success = []
        self._testsRun = 0
        self._currentTest = None
        super(ParameterisedTestResult,self).__init__( )

    def StartTest(self, test):
        self._currentTest = test
        super(ParameterisedTestResult,self).StartTest( test )

    def StopTest(self, test):
        self._currentTest = None
        super(ParameterisedTestResult,self).StopTest( test )
        self._testsRun += self._testsRun

    def addSuccess(self, test):
        self._success.append( (test.id(), test) )
...
class TestMethodGeneration(unittest.TestCase):
    ...
    def _generate_test_method_name(test_name, num_test_cases):
        for i in range(0, num_test_cases):
            yield "test_{index:03d}_{test_name}".format(index=i, test_name=test_name)
    ...
    def test_010_MethodGenerationTestNames(self):
        ...
        for index, test_method_name in enumerate(self._generate_test_method_name(
                                                test_name,
                                                num_test_cases ) ):
            ...
            self.assertIsNotNone( method,
                    msg='Failed to generate the correct test_method'
    ...
    def test_020_MethodGenerationDocStrings(self):
        """Test that test_method with the correct documentation strings have been generated"""
        test_name = "MethodGeneration"
        num_test_cases = 3
        ...
        for index, test_info in enumerate(zip(
            ...
            # Extract associated name and data - and get the actual method object
...
class TestMethodExecution(unittest.TestCase):
    ...
    def _run_tests( test_class ):
        ...
        summary = result.testsRun, len(result.errors), len(result.failures), len(result.skipped), len(result.expectedFailures), len(result.unexpectedSuccesses)

        return summary, result

    def test_001_MethodExecution(self):
        """Test that test_method will execute correctly - the all should pass"""
    ...
    def test_010_MethodExecutionOneFail(self):
        ...
        case_cls_ = RepeatedTestFramework(
                        ...
                        test_cases = [{'a':1, 'b':2},
                                      {'a': 2, 'b': 3},
                                      {'a': 3, 'b': 3},],
            method_doc_template="""{test_name} {index:03d}: {a} {b}""" )( self.cls_ )
        ...
        self.assertEqual( result.failures[0][0].id(),
                    'tests.test_repeatedtestframework.EmptyClass.test_002_MethodExecution'
                          )
    ...
    def test_015_MethodExecutionTwoFail(self):
        ...
        self.assertEqual( result.failures[0][0].id(),
                    'tests.test_repeatedtestframework.EmptyClass.test_001_MethodExecution'
                          )
    ...
    def test_100_DecoratorsErrorsInvalidCriteria(self):
        ...
        with six.assertRaisesRegex(self, TypeError, r'.*criteria.*not callable.*'):
    ...
    def test_101_DecoratorsErrorsInvalidDecoratorMethod(self):
        """Confirm that the decorator_method attribute is checked to be a callable"""
        with six.assertRaisesRegex(self, TypeError, r'.*decorator_method.*not callable.*'):
            skip_dec = DecorateTestMethod(
                criteria=lambda x:x,
    ...
    def test_110_DecoratorsOrderErrors(self):
        """Confirm error is generated when attempt is made to use DecorateTestMethod before RepeatedTestFramework"""
        skip_dec = DecorateTestMethod(
            criteria=lambda data: data['a'] == 1,
            decorator_method=unittest.skip,
            decorator_kwargs={'reason': 'Skipped because a == 1'})

        with six.assertRaisesRegex(self, TypeError, r'Incorrect usage; DecorateTestMethod.*'):
    ...
    def test_120_SkipTest(self):
        ...
        self.assertEqual(result.skipped[0][0].id(),
                         'tests.test_repeatedtestframework.EmptyClass.test_000_MethodExecution'
                         )
    ...
    def test_150_SkipIfShortcutTestConditionTrue(self):
        """Confirm that the skipIf shortcut decorator works as expected when condition == True"""
        test_name = 'MethodExecution'
        test_value = 3
        skip_dec = skipIf(
    ...
    def test_155_SkipIfShortcutTestConditionFalse(self):
        """Confirm that the skipIf shortcut decorator works as expected when condition == False"""
        test_name = 'MethodExecution'
        test_value = 3
        skip_dec = skipIf(
    ...
    def test_165_SkipUnlessShortcutTestConditionFalse(self):
        """Confirm that the SkipUnless shortcut decorator works as expected when condition == False"""
        test_name = 'MethodExecution'
        test_value = 3
        skip_dec = skipUnless(
                condition= (test_value >= 2),
    ...
    def test_170_ExpectedFailureShortcutTest(self):
        """Confirm that the ExpectedFailure shortcut decorator works as expected"""
        test_name = 'MethodExecution'
        expectedFail = expectedFailure(
                criteria=lambda data: data['a'] == 1 )
        ...
        case_cls_ = expectedFail(case_dec_(self.cls_))
        summary, result = self._run_tests(case_cls_)
    ...
    def test_170_ExpectedFailureShortcutTestUnexpecteSuccess(self):
        """Confirm that the ExpectedFailure shortcut decorator works as expected when the test works"""
        test_name = 'MethodExecution'
        expectedFail = expectedFailure(
                criteria=lambda data: data['a'] == 1 )
